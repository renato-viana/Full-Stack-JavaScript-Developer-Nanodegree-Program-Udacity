...
const userOptions = async (mochaObject) => {
  ...
  const milkChoice = parseInt(milkPicked, 10);
  const espPicked = await espressoQ();
  const espChoice = parseInt(espPicked, 10);
  // If peppermint mocha
  if (mochaObject instanceof PeppermintMocha) {
    const pepPicked = await peppermintQ();
    const pepChoice = parseInt(pepPicked, 10);
...
const main = async () => {
  ...
  const buildMocha = () => {
    do {
      const optionPicked = showMenu();
      menuChoice = parseInt(optionPicked, 10);
      switch (menuChoice) {
        case 0: {
          break;
        }
